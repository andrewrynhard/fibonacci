...
func (o *GetSequenceURL) Build() (*url.URL, error) {
	...
	var _path = "/sequence/{n}"

	n := swag.FormatInt64(o.N)
	if n != "" {
		_path = strings.Replace(_path, "{n}", n, -1)
	} else {
		return nil, errors.New("N is required on GetSequenceURL")
	}
...
func (o *GetSequenceURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetSequenceURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetSequenceURL")
	}
